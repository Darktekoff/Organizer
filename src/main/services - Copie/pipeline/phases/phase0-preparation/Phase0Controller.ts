...
export class Phase0Controller implements PhaseController<Phase0Input, Phase0Data> {
  ...
  async execute(
    ...
    try {
      ...
      if (alreadyReorganized) {
        console.log('✅ Folder already reorganized! Skipping to snapshot creation...');
        ...
        if (existingData) {
          // Créer directement le snapshot enrichi POST-réorganisation
          console.log('📸 Creating enriched POST-reorganization snapshot...');
          this.capturePostReorganizationSnapshot(
            existingData.workingPath || input.sourcePath,
            existingData
          );

          console.log('✅ Snapshot enriched created without re-reorganization!');
  ...
  private checkIfAlreadyReorganized(sourcePath: string): boolean {
    try {
      ...
      const hasOriginalSnapshot = fs.existsSync(path.join(snapshotDir, 'structure-originale.json'));
      const hasProposedStructure = fs.existsSync(path.join(snapshotDir, 'structure-proposee.json'));
      const hasDetectionSnapshot = fs.existsSync(path.join(snapshotDir, 'structure-detection.json'));

      // Si on a les 3 fichiers, c'est que la réorg a été faite
      const isReorganized = hasOriginalSnapshot && hasProposedStructure && hasDetectionSnapshot;

      if (isReorganized) {
        console.log('🔍 Detected previous reorganization markers:');
        console.log('  ✅ structure-originale.json found');
        console.log('  ✅ structure-proposee.json found');
        console.log('  ✅ structure-detection.json found');

        // Vérifier aussi qu'on n'a PAS déjà le snapshot enrichi
        const hasEnrichedSnapshot = fs.existsSync(path.join(snapshotDir, 'structure-reorganized.json'));
        if (hasEnrichedSnapshot) {
          console.log('  ✅ structure-reorganized.json already exists!');
        } else {
          console.log('  ⚠️ structure-reorganized.json missing - will create it');
        }
      }

      return isReorganized;
