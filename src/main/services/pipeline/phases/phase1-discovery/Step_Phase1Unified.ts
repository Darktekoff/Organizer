...
import type { DetectedPackV6, EnrichedPack, BundleInfo } from '@shared/interfaces/BusinessTypes';
...
export class Step_Phase1Unified implements StepExecutor<Phase1UnifiedInput, Phase1UnifiedOutput> {
  ...
  async execute(
    ...
    try {
      ...
      const enrichedPacks = this.convertPackIndexToEnrichedPacks(packIndex, fileEntries, bundleMap);
  ...
  private convertPackIndexToEnrichedPacks(
    ...
    allFiles: FileEntry[],
    bundleMap: Map<string, BundleInfo>
  ): EnrichedPack[] {
    ...
    if (packIndex.size === 0) {
      ...
      return this.reconstructPacksFromFiles(allFiles, bundleMap);
    }

    const enrichedPacks: EnrichedPack[] = [];

    for (const [packId, packFiles] of packIndex.entries()) {
      const enrichedPack = this.createEnrichedPack(packId, packFiles, bundleMap);
   ...
   */
  private reconstructPacksFromFiles(files: FileEntry[], bundleMap: Map<string, BundleInfo>): EnrichedPack[] {
    ...
    return Array.from(packGroups.entries()).map(([packId, packFiles]) =>
      this.createEnrichedPack(packId, packFiles, bundleMap)
    );
  }
   ...
   */
  private createEnrichedPack(packId: string, packFiles: FileEntry[], bundleMap: Map<string, BundleInfo>): EnrichedPack {
    ...
    return {
      ...
      // Informations de bundle pour classification contextuelle
      bundleInfo: bundleMap.get(packId) || undefined
    };
  }
  ...
  private async loadBundleHierarchy(workingPath: string): Promise<Map<string, BundleInfo>> {
    ...
    try {
      const detectionPath = path.join(workingPath, '.audio-organizer', 'structure-detection.json');
      const detectionExists = await fs.access(detectionPath).then(() => true).catch(() => false);

      if (!detectionExists) {
        console.warn('⚠️ structure-detection.json not found, bundle context unavailable');
        return bundleMap;
      }
      ...
      // Parcourir récursivement la structure pour identifier les bundles et leurs packs
      this.extractBundleHierarchy(detectionData, bundleMap);

      return bundleMap;
    } catch (error) {
      console.error('❌ Error loading bundle hierarchy:', error);
      return bundleMap;
    }
  }

  /**
   * Extrait la hiérarchie bundle → packs depuis les données de la Phase 0
   * (Pas de re-détection, on utilise ce que Phase 0 a déjà trouvé)
   */
  private extractBundleHierarchy(node: any, bundleMap: Map<string, BundleInfo>, parentBundleName?: string): void {
      ...
      if (child.packType === 'BUNDLE' || child.packType === 'COMMERCIAL_BUNDLE') {
        ...
        if (siblingPacks.length > 0) {
          const bundleInfo: BundleInfo = {
            bundleName: childName,
            bundlePath: child.path,
            siblingPacks,
            bundleKeywords: this.extractKeywordsFromName(childName)
          };

          // Associer tous les packs enfants à ce bundle
          for (const packName of siblingPacks) {
            bundleMap.set(packName, bundleInfo);
          }

          console.log(`📦 Bundle from Phase 0: "${childName}" with ${siblingPacks.length} packs`);
        }
      }
  ...
  /**
   * Extrait les mots-clés importants d'un nom de bundle pour la classification
   */
  private extractKeywordsFromName(name: string): string[] {
    ...
    // Styles musicaux
    const musicStyles = [
      'hardstyle', 'rawstyle', 'hardcore', 'uptempo', 'frenchcore',
      'dubstep', 'future bass', 'trap', 'drum bass', 'dnb',
      'techno', 'house', 'trance', 'psytrance',
      'hip hop', 'rap', 'r&b', 'phonk'
    ];
    ...
    // Types de contenu
    const contentTypes = [
      'kick', 'bass', 'vocal', 'melody', 'chord', 'lead',
      'pad', 'arp', 'fx', 'percussion', 'drum'
    ];
